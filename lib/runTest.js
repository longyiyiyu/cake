...
function compile(pwd) {
  ...
  utils.walkDirs(questionDirPath, (dir) => {
    utils.babelTransform(path.join(questionDirPath, dir), path.join(pwd, cfg.TMPDIRNAME, cfg.TMPQUESTIONDIR, dir));
  });

  // 编译答案
  utils.walkDirs(pwd, (dir) => {
    utils.babelTransform(path.join(pwd, dir), path.join(pwd, cfg.TMPDIRNAME, cfg.TMPANSWERDIR, dir));
  });
}
...
function run(pwd, type) {
  // 检测是否有自定义测试用例
  const customTestFilePath = path.join(pwd, cfg.TMPDIRNAME, cfg.TMPANSWERDIR, type, cfg.CUSTOMTESTFILENAME);
  ...
  const questionConfigPath = path.join(pwd, '../..', cfg.QUESTIONDIRNAME, cfg.QUESTIONCONFIGFILENAME);
...
function runTest(id, userName, type) {
  ...
  const quizDir = path.join(root, cfg.ALLQUIZZESDIRNAME, `${cfg.QUIZDIRNAMEPREFIX}${id}`);
